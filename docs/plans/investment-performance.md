...
- [x] (2026-08-15 21:40Z) Follow-on: asset-allocation donut. `computeAllocation` groups all holdings (cash and basis-less included — allocation is about where money sits, not what it gained) by the security type stored in the cash-split follow-on; the panel renders a donut + legend with fixed type→color mapping (cash neutral gray) between the chart and the by-account section. UI + API only, no migration; deployed same day. Production shares at ship: single stocks 51.6%, ETFs 31.7%, options 10.1%, cash 3.5%, mutual fund 3.1%.

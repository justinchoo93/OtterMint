...
- [ ] Milestone 1: pure math — `computeAccountNetGains` in `src/lib/investment-performance.ts` with unit tests.
- [ ] Milestone 2: API — extend `GET /api/analytics/investments` with an `accountReturns` field, with route tests.
- [ ] Milestone 3: UI — "Net Gain by Account" section in `InvestmentPerformancePanel`, with component tests.
...
- Decision: The lifetime gate is three conditions, all required: (a) the cash residual is exactly zero — the account's live `available_balance` equals the sign-flipped sum of every stored feed row for the account; (b) the account's earliest stored feed row is an external cash-in (an external-flow subtype with negative amount); (c) that earliest row is dated at least 7 days after the account's backfill window start, taken as the Plaid item's `created_at` minus 24 months.
...
Reconciliation queries, run from any machine with NAS access (read-only; also the drift check to re-run whenever a new institution is linked):
    ...
    SELECT a.name, a.mask, -sum(it.amount) AS implied_cash, a.available_balance,
           a.available_balance + sum(it.amount) AS residual
...
Expected today: residual 0.00 for Individual ····5111 and Roth ····6093; large positive residuals for the Self-Directed pair; no external-flow-like subtype outside the plan's set.
...
No new libraries. In `src/lib/investment-performance.ts` (pure, no server imports), define and export:
    ...
    export interface AccountInfoRow {
      ...
      availableBalance: string | null;  // investment accounts: uninvested cash
...
`src/app/api/analytics/investments/route.ts` extends `InvestmentsResponse` with `accountReturns: AccountNetGain[]` and supplies the three row sets described in the Plan of Work. `src/components/dashboard/InvestmentPerformancePanel.tsx` consumes `accountReturns` and renders the section; its `EMPTY` constant gains `accountReturns: []`.

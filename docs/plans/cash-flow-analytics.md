...
- [x] (2026-08-14 00:15Z) Production migration + deploy. The NAS is reachable over `ssh otterholt` (the earlier "no database access" finding applied only to the dev machine's localhost URL). Applied migration 0010 by hand inside `ottermint-db-1` — `ALTER TABLE` plus the drizzle journal row (`hash f5ae86af…`, `created_at 1786663746495`), after verifying the method reproduces drizzle's recorded hash for 0009 exactly. Deployed via `scripts/deploy.sh`; health `{"status":"ok","db":"ok"}`; `/api/analytics/cashflow` live (401 unauthenticated, as designed). Backfill armed: 6 plaid_items cursors and 11 accounts' `last_refreshed_at` nulled; 428 transactions have `category_detailed = NULL` pending re-sync. Method documented in `docs/DEPLOYMENT.md §5.5`; OtterHolt.md's stale OtterMint entry corrected.
- [x] (2026-08-14 00:40Z) Backfill executed and verified. After one Refresh press: 434/434 transactions have `category_detailed` (zero nulls; 6 new transactions arrived with the re-sync), all 6 cursors re-established. Spot-check confirms the motivating distortions and their magnitudes: "Transfer Out" was $67,700 of investment contributions (34 txns) miscounted as spending; "Loans" was 24 credit-card payments ($6,938.14), all previously double-counted; FOOD_AND_DRINK splits into six detailed subcategories (restaurant $2,683 vs groceries $2,101, etc.).
- [ ] Browser eyeball of the Analytics view by Justin (data-side acceptance is fully verified; the UI renders from the verified endpoint).
...
- Observation: Production data contains Plaid detailed values outside the taxonomy documented in this plan: `TRANSFER_IN_TRANSFER_IN_FROM_APPS` and `TRANSFER_OUT_TRANSFER_OUT_FROM_APPS` (payment-app transfers, Venmo/CashApp-style), and the restaurant value is singular `FOOD_AND_DRINK_RESTAURANT`, not `RESTAURANTS`; payroll arrives as `INCOME_SALARY`, not the documented `INCOME_WAGES`. The primary-prefix fallback classified the app transfers exactly as designed (outflow → spending, inflow → internal), and prefix-stripping labels are unaffected by the singular/plural difference — the "not exhaustive" design assumption proved out on day one.
  Evidence: backfill spot-check 2026-08-14, `SELECT category, category_detailed, count(*) ... GROUP BY 1,2`.
...
(2026-08-14, shipped) Deployed to production and verified against real data. The purpose is met with measured magnitudes: the old spending chart had been inflating spending by $67,700 of investment contributions plus $6,938 of double-counted credit-card payments over the retained history — together far exceeding actual discretionary spending, which explains why the analytics "felt wrong" to the user. The backfill reached 100% detailed-category coverage in one Refresh press. Two lessons beyond the local-database one: (a) the migration path for this stack was undocumented and is now `docs/DEPLOYMENT.md §5.5` — the standalone runtime image can never run drizzle-kit, so hand-applied psql + journal-row is the canonical production method; (b) Plaid's detailed taxonomy drifts from any documented list (`*_FROM_APPS`, singular `RESTAURANT`), so classification fallbacks are not defensive nicety but a day-one requirement. Remaining: nothing in code or data; Justin eyeballs the Analytics view at his leisure.

...
- Observation: Production data contains Plaid detailed values outside the taxonomy documented in this plan: `TRANSFER_IN_TRANSFER_IN_FROM_APPS` and `TRANSFER_OUT_TRANSFER_OUT_FROM_APPS` (payment-app transfers, Venmo/CashApp-style), and the restaurant value is singular `FOOD_AND_DRINK_RESTAURANT`, not `RESTAURANTS`. The primary-prefix fallback classified the app transfers exactly as designed (outflow → spending, inflow → internal), and prefix-stripping labels are unaffected by the singular/plural difference — the "not exhaustive" design assumption proved out on day one.

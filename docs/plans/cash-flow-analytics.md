...
- [ ] Backfill execution: Justin presses Refresh once in the app (manual refresh only — production has no Plaid webhooks), then verify `count(category_detailed)` ≈ 428 and the category spot-check query, then the three browser acceptance scenarios.
